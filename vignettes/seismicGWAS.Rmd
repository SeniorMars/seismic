---
title: "An Introduction to the seismicGWAS Package"
...
sections: Section 1 contains a walkthrough using the sample data included with
...
`seismicGWAS` includes a small preloaded single cell RNAseq dataset in the
...
identifiers. For convenience, `seismicGWAS` has a function for translating between
...
With `seismicGWAS`, we can also look at the genes driving a particular cell
type-trait association in a process we call influential gene analysis. Here,
we must select a particular cell type (in our example, we choose
pancreas beta cell) and a trait (we use Type 2 Diabetes again). It is important
to note that influential gene calculations are only meaningful for significant cell
type-trait associations. We chose a cell type and trait that were found to be 
...
This section illustrates how to run the `seismicGWAS` package on a real-world single cell dataset from Tabula Muris. 
For convenience, we use the [`scFind`](https://github.com/hemberg-lab/scfind) package to grab
...
calculating cell specificity scores. Currently, there are some `NA` values for
field. Let's remove them before continuing.
